...
一个基于 Web 的提示词工程与对话质量评测平台，支持多模型、多服务商、场景化配置、批量并行测试与自动化评估，适合课程/毕业设计、团队内部评测与日常 Prompt 实验。

## 🌟 功能特性
- 主对话与流式输出：实时对话、消息选择与复制、自动/连续响应、结束条件控制。
- 场景配置系统：YAML 可视化配置、模板替换、LLM 生成或直接输出三种生成模式，实时预览与一键应用。
...
## 🏗️ 技术栈与架构
- 前端：原生 HTML/CSS/JS，模块化组织，纯静态托管即可运行。
- 后端：Node.js + Express（ESM 模块），JWT 认证，CORS 支持。
- 数据库：MySQL（用户、提示词、API Key、实验/测试结果）。
...
- 场景配置与 YAML 编辑：`js/scene-config/*`, `js/scene-config/yaml-editor/*`
...
JWT_EXPIRES_IN=7d
```
说明：`FRONTEND_URL` 需与前端访问地址一致；`DB_*` 为 MySQL 连接信息；`JWT_*` 为认证配置。
...
安全提示：示例实现将密钥明文存储于数据库，仅供演示。生产环境请结合 KMS/密钥加密、细粒度权限、审计等措施。
...
## 🧪 使用建议
- 先在“模型设置”配置服务商与 API Key，再在“主对话”和“场景配置”中迭代 Prompt。  
- 批量评测用“并行测试”，结合“智能分析”固化 JSON 输出，便于自动比对。  
- 如需修改后端地址或 CORS，请同步调整 `backend/src/app.js` 和 `js/api-manager.js`。
...
## 📄 License
- 根目录 `package.json` 标注 `ISC`，后端 `backend/package.json` 标注 `MIT`。若需统一协议，请在根目录新增 `LICENSE` 并保持一致。

---
如果本项目对你有帮助，欢迎 Star，亦可提 Issue/PR 一起改进。
